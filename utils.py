...
def plotting_save(history, true_label, predict_label, model_name, stage, times):
    ...
    if stage=='val':
        np.savetxt('results\\VAL_data_'+model_name+'.csv', model_data, delimiter=';', fmt='%s')    
        np.savetxt('results\\VAL_pred_'+model_name+'.csv',pred_data, delimiter=';',fmt='%s')
        np.savetxt('results\\VAL_fpr_tpr_'+model_name+'.csv',fpr_tpr, delimiter=';')
        np.savetxt('results\\times_'+model_name+'.csv',times, delimiter=';')

    else:
        np.savetxt('results\\data_'+model_name+'.csv', model_data, delimiter=';', fmt='%s')    
        np.savetxt('results\\pred_'+model_name+'.csv',pred_data, delimiter=';',fmt='%s')
        np.savetxt('results\\VAL_fpr_tpr_'+model_name+'.csv',fpr_tpr, delimiter=';')
